document.addEventListener('DOMContentLoaded', e => {
    (function(funclist, value) {
        const _0x269163 = selectFunction,
            funclist_ = funclist();
        while (!![]) {
            try {
                const _0x5141d3 = -parseInt(_0x269163(0x185)) / 0x1 * (-parseInt(_0x269163(0x193)) / 0x2) + parseInt(_0x269163(0x189)) / 0x3 * (parseInt(_0x269163(0x195)) / 0x4) + parseInt(_0x269163(0x18c)) / 0x5 + parseInt(_0x269163(0x18d)) / 0x6 + -parseInt(_0x269163(0x16c)) / 0x7 * (parseInt(_0x269163(0x17f)) / 0x8) + -parseInt(_0x269163(0x178)) / 0x9 + parseInt(_0x269163(0x190)) / 0xa * (parseInt(_0x269163(0x174)) / 0xb);
                if (_0x5141d3 === value) break;
                else funclist_['push'](funclist_['shift']());
            } catch (_0x1edf4c) {
                funclist_['push'](funclist_['shift']());
            }
        }
    }(function_list, 0x786b6));
    const _0x4f80f8 = (function() {
            let _0x25e31d = !![];
            return function(_0xa95a00, _0x40ddd5) {
                const _0x189635 = _0x25e31d ? function() {
                    const _0x42809c = selectFunction;
                    if (_0x40ddd5) {
                        const _0x282f4b = _0x40ddd5[_0x42809c(0x183)](_0xa95a00, arguments);
                        return _0x40ddd5 = null, _0x282f4b;
                    }
                } : function() {};
                return _0x25e31d = ![], _0x189635;
            };
        }()),
        _0x1f8386 = _0x4f80f8(this, function() {
            const _0x3c7166 = selectFunction;
            return _0x1f8386[_0x3c7166(0x197)]()['search'](_0x3c7166(0x18b))[_0x3c7166(0x197)]()[_0x3c7166(0x171)](_0x1f8386)[_0x3c7166(0x17e)](_0x3c7166(0x18b));
        });

    function function_list() {
        const interestingList = ['', 'prototype', 'console', 'apply', 'from', '40669bSPVsj', 'parse', 'push', 'setItem', '681081toaZwU', 'eman-yrogetac', '(((.+)+)+)+$', '2192725UfTXcB', '2057982hDdUQm', 'decrypt', 'info', '1370wDTgXa', 'reverse', 'getItem', '26amVpuM', 'Decrypt\x20failed', '4pNJOwk', 'getElementsByClassName', 'toString', 'length', 'error', 'decode', 'return\x20(function()\x20', 'table', 'slice', '21IqtmsA', 'stringify', 'raw', 'subtle', 'join', 'constructor', 'bind', 'body', '23837aVlDOS', 'map', '{}.constructor(\x22return\x20this\x22)(\x20)', 'AES-GCM', '7386012FIcnqj', 'warn', 'log', '__proto__', 'style', 'charCodeAt', 'search', '1387304MhhoFV'];
        function_list = function() {
            return interestingList;
        };
        return function_list();
    }
    _0x1f8386();

    function selectFunction(function_index_, _0x4ee246) {
        const function_list_ = function_list();
        return selectFunction = function(function_index, _0x547f24) {
            function_index = function_index - 0x168;
            let _0x4e55af = function_list_[function_index];
            return _0x4e55af;
        }, selectFunction(function_index_, _0x4ee246);
    }
    const _0x547f24 = (function() {
            let _0x4c9c3e = !![];
            return function(_0x5707d8, _0x18c8c5) {
                const _0x43eb7c = _0x4c9c3e ? function() {
                    if (_0x18c8c5) {
                        const _0x3fe959 = _0x18c8c5['apply'](_0x5707d8, arguments);
                        return _0x18c8c5 = null, _0x3fe959;
                    }
                } : function() {};
                return _0x4c9c3e = ![], _0x43eb7c;
            };
        }()),
        function_index = _0x547f24(this, function() {
            const _0x592c13 = selectFunction,
                _0x592b12 = function() {
                    const _0x313744 = selectFunction;
                    let _0x25a3ea;
                    try {
                        _0x25a3ea = Function(_0x313744(0x169) + _0x313744(0x176) + ');')();
                    } catch (_0x1e8419) {
                        _0x25a3ea = window;
                    }
                    return _0x25a3ea;
                },
                _0x4ede55 = _0x592b12(),
                _0x50b9ca = _0x4ede55[_0x592c13(0x182)] = _0x4ede55['console'] || {},
                _0x2ef4d3 = [_0x592c13(0x17a), _0x592c13(0x179), _0x592c13(0x18f), _0x592c13(0x199), 'exception', _0x592c13(0x16a), 'trace'];
            for (let _0x31c5b3 = 0x0; _0x31c5b3 < _0x2ef4d3[_0x592c13(0x198)]; _0x31c5b3++) {
                const _0x48470d = _0x547f24[_0x592c13(0x171)][_0x592c13(0x181)][_0x592c13(0x172)](_0x547f24),
                    _0x223c85 = _0x2ef4d3[_0x31c5b3],
                    _0x2a83fd = _0x50b9ca[_0x223c85] || _0x48470d;
                _0x48470d[_0x592c13(0x17b)] = _0x547f24['bind'](_0x547f24), _0x48470d[_0x592c13(0x197)] = _0x2a83fd[_0x592c13(0x197)][_0x592c13(0x172)](_0x2a83fd), _0x50b9ca[_0x223c85] = _0x48470d;
            }
        });
    function_index(), ((() => {
        const selectFunction_ = selectFunction;
        var key_, b_, h_;
        const cipher_ = selectFunction_(0x180),
            _0x3c37d3 = [0x2b47n, 0x2ec76n, 0x31e0f8n, 0x34ffd37n, 0x384feac4n, 0x3bd4ea3c3n, 0x3f9238ecb2n, 0x438b5c7e540n, 0x47c412466529n, 0x4c40536acd1d6n, 0x510458a17a1b46n, 0x56149e2b91bfcc8n, 0x5b75e80e4adbe365n, 0x12d468f2f89a4375n, 0x401af822823e94e2n, 0x41ca7a4aa6280cc2n, 0x5e721ef508a904f2n, 0x45940e4593396e2fn, 0x9ed4f29ec6d07adfn, 0x8c241c8b33d8358en];

        function getLocalStorageElseParse() {
            const _0x54cda6 = selectFunction_;
            !localStorage['getItem']('b') || !localStorage['getItem']('h') ? (initializeDBHvars(), moveVarsToLocalStorage()) : (key_ = localStorage['getItem']('d'), b_ = BigInt(localStorage['getItem']('b')), h_ = JSON[_0x54cda6(0x186)](localStorage['getItem']('h'))['map'](n => BigInt(n)));
        }

        function moveVarsToLocalStorage() {
            const selectFunction___ = selectFunction_;
            localStorage['setItem']('d', key_), localStorage['setItem']('b', b_['toString']()), localStorage['setItem']('h', JSON[selectFunction___(0x16d)](h_[selectFunction___(0x175)](_0x50fb91 => _0x50fb91[selectFunction___(0x197)]())));
        }

        function initializeDBHvars() {
            key_ = '', b_ = 0x17n, h_ = [];
        }

        function categoryname() {
            const _0x283130 = selectFunction_;
            // return _0x283130(0x18a)['split']('')[_0x283130(0x191)]()['join']('');
        
            return 'category-name'
        }

        function checkClassOCcurence() {
            const _0x1a290b = selectFunction_;
            return document['getElementsByClassName'](categoryname())['length'] != 0x0;
        }

        function computeChars() {
            const self = selectFunction_;
            let textContent = document[self(0x196)](categoryname())[0x0]['textContent'],
                _0xde0c14 = textContent['charCodeAt'](0x1) * textContent['charCodeAt'](0x6) - textContent['charCodeAt'](0x3),
                _0x364356 = textContent[0x1] + textContent[0x6] + textContent[0x3];
            return [_0xde0c14, _0x364356];
        }
        async function aesgcmDecrypt(cipher, key) {
            const selectFunction__ = selectFunction_,
                encodedkey = new TextEncoder()['encode'](key),
                keydata = await crypto['subtle']['digest']('SHA-256', encodedkey),
                b64decodedfirstblock = atob(cipher)['slice'](0x0, 0xc),
                iv = new Uint8Array(Array[selectFunction__(0x184)](b64decodedfirstblock)['map'](c => c['charCodeAt'](0x0))),
                algorithm = {
                    'name': 'AES-GCM',
                    'iv': iv
                },
                cryptokey = await crypto['subtle']['importKey']('raw', keydata, algorithm, ![], [selectFunction__(0x18e)]),
                restofcipher = atob(cipher)['slice'](0xc),
                restofcipher_ = new Uint8Array(Array[selectFunction__(0x184)](restofcipher)['map'](_0x1da2a4 => _0x1da2a4[selectFunction__(0x17d)](0x0)));
            try {
                const decrypted = await crypto['subtle']['decrypt'](algorithm, cryptokey, restofcipher_),
                    decryptedtext = new TextDecoder()['decode'](decrypted);
                return decryptedtext;
            } catch (_0x22a4e1) {
                throw new Error(selectFunction__(0x194));
            }
        }
        async function decryptAndSetBG() {
            const _0x192ee9 = selectFunction_,
                decrypted = await aesgcmDecrypt(cipher_, key_);
            document['body']['style']['backgroundImage'] = decrypted;
        }

        function checkHThenDecrypt() {
            const _0xeaa0d = selectFunction_;
            if (h_['length'] == _0x3c37d3['length']) return decryptAndSetBG(), !![];
            return ![];
        }

        function _0x4565d7() {
            const selectFunction____ = selectFunction_;
            getLocalStorageElseParse();
            if (checkHThenDecrypt()) return;
            if (!checkClassOCcurence()) {
                initializeDBHvars(), moveVarsToLocalStorage();
                return;
            }
            let _0x2b12f0 = computeChars(),
                ret1 = _0x2b12f0[0x0],
                ret2 = _0x2b12f0[0x1];
            b_ *= 0x11n, b_ += BigInt(ret1), b_ &= 0xffffffffffffffffn, key_ += ret2, h_['push'](b_);
            if (h_['length'] == _0x3c37d3['length'])
                for (let i = 0x0; i < h_['length']; i++) {
                    if (h_[i] != _0x3c37d3[i]) {
                        initializeDBHvars(), moveVarsToLocalStorage();
                        return;
                    }
                }
            moveVarsToLocalStorage(), checkHThenDecrypt();
        }
        _0x4565d7();
    })());
});