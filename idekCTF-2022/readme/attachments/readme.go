...
func justReadIt(w http.ResponseWriter, r *http.Request) {
	...
	for _, o := range reqData.Orders {
		...
		ctx = WithValueValidatorCtx(ctx, reader, int(o))
...
func (v *Validator) Validate(ctx context.Context) error {
	...
	buf, err := v.Read(WithValueValidatorCtx(ctx, r, 32))
	if err != nil {
		return err
	}
	if bytes.Compare(buf, password[:]) != 0 {
		return errors.New(fmt.Sprintf("invalid pass: %s\n", buf))
	}
	return nil
}
...
func WithValueValidatorCtx(ctx context.Context, r io.Reader, size int) context.Context {
