...
try:
    for _ in range(ROUNDS):
        alice_token = list(os.urandom(16))
        msg = alice(alice_token)
        print("MSG: ",msg)
        assert type(msg) == list and len(msg) <= 64 and all(x in range(256) for x in msg)
        random.shuffle(msg)
        bob_token = bob(msg)
        print("Bob:",bob_token)

        print(bob_token,alice_token)
